...
;up_mcr_depart remove ?f1
(:action dynamicmacro_mod_1 :parameters (?f2 - floor ?px2 - passenger)
:precondition (and
(destin ?px2 ?f2)
(boarded ?px2)
)
:effect (and (served ?px2)
(not (boarded ?px2))
)
)
 
;board_mcr_up_mcr_depart remove ?f2x2
(:action dynamicmacro_mod_6 :parameters (?f - floor ?p - passenger)
:precondition (and
(origin ?p ?f)
)
:effect (and (served ?p)
